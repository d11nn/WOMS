...
const state = {
  ...
  mobileView: "orders",
  selectedLine:
    localStorage.getItem("woms.selectedLine") || defaultLine(fallbackLines),
  filters: {
    customers: new Set(),
    status: "",
    priorities: new Set(),
  },
  calendarDate: calendarDateFromDateKey(
    dateKeyInTimeZone(new Date(), defaultTimezone),
  ),
};
...
document.querySelector('input[name="dueDate"]').value = addDaysToDateKey(
  todayDateInputValue(),
  3,
);
syncDueDateMinimums();
...
document
  .getElementById("active-line-select")
...
document
  .getElementById("assign-user-form")
  .addEventListener("submit", async (event) => {
    event.preventDefault();
    try {
      const data = Object.fromEntries(new FormData(event.currentTarget));
      ...
      showMessage(
        "帳號已更新",
        `${user.username} 現在是${roleLabel(user.role)}${user.lineId ? `，產線 ${user.lineId}` : ""}`,
      );
      await loadUsers();
    } catch (error) {
...
document
  ...
  .addEventListener("submit", async (event) => {
    ...
    try {
      ...
      showMessage(
        "帳號已建立",
        `${user.username} 可用 ${roleLabel(user.role)} 權限登入`,
      );
      await loadUsers();
    } catch (error) {
...
document
  ...
  .addEventListener("click", async () => {
    ...
    try {
      const user = await request(`/api/users/${encodeURIComponent(username)}`, {
        method: "DELETE",
      });
      showMessage(
        "帳號已處理",
        user.deleted ? `${username} 已刪除` : `${user.username} 已停用`,
      );
      await loadUsers();
    } catch (error) {
...
document
  .getElementById("create-hpa-peak")
  .addEventListener("click", async () => {
    try {
      ...
      syncHPAPeakPolling();
      showMessage(
        "web autoscaling demo 已載入",
        "請由 NGINX Ingress 或 LoadBalancer 送入多使用者流量，觀察 Grafana per-pod NGINX req/s 與 web HPA。",
      );
    } catch (error) {
...
document
  .getElementById("refresh-hpa-peak")
  .addEventListener("click", async () => {
    await loadHPAPeakSummary();
  });

document
  .getElementById("clear-hpa-peak")
  .addEventListener("click", async () => {
    const ok = window.confirm(
      "確定要清除舊版 L001-L200 排程尖峰資料嗎？web traffic autoscaling demo 本身不需要建立訂單或任務。",
    );
    if (!ok) {
      return;
    }
    try {
      ...
      showMessage(
        "舊版資料已清除",
        "web autoscaling demo 請改用 Ingress/LoadBalancer 壓測與 Grafana 指標觀察。",
      );
      await refreshWorkspace();
    } catch (error) {
...
document
  .getElementById("main-calendar-mode")
...
document
  .getElementById("preview-calendar-mode")
  .addEventListener("click", (event) => {
...
document
  .getElementById("preview-selected")
  .addEventListener("click", async () => {
...
document
  .getElementById("cancel-selected")
  .addEventListener("click", async () => {
    if (state.selectedOrderIds.size === 0) {
    ...
    const ok = window.confirm(
      `確定取消 ${state.selectedOrderIds.size} 張已選取訂單嗎？`,
    );
    if (!ok) {
      return;
    }
    try {
      ...
      showMessage(
        "取消完成",
        `已取消 ${payload.cancelledOrderIds?.length ?? 0} 張訂單。`,
      );
      await refreshWorkspace();
    } catch (error) {
...
document
  ...
  .addEventListener("submit", async (event) => {
    ...
    await submitProductionReport(
      form.orderId,
      form.productionDate,
      Number(form.producedQuantity),
    );
  });

document
  .getElementById("cancel-production-report")
  .addEventListener("click", () => {
    closeProductionReport();
  });

document
  .getElementById("confirm-reject-orders")
  .addEventListener("click", async () => {
...
document
  .getElementById("scheduler-panel-toggle")
  ?.addEventListener("click", () => {
    setSchedulerPanelOpen(!state.schedulerPanelOpen);
  });

document
  .getElementById("create-conflict-demo")
  .addEventListener("click", async () => {
    try {
      ...
      showMessage(
        "衝突測試訂單已建立",
        `已建立 ${payload.orders?.length ?? 0} 張同日大量訂單，現在可以選取它們並試排。`,
      );
      await refreshWorkspace();
    } catch (error) {
...
document
  .getElementById("confirm-preview-order")
  .addEventListener("click", async () => {
...
document
  .getElementById("confirm-schedule-job")
  .addEventListener("click", async () => {
    if (!state.preview?.previewId) {
      return;
    }
    ...
    if (manualForce && !conflictsCanBeManuallyForced(conflicts)) {
      showMessage(
        "無法人工介入",
        "容量無法在交期前排完的衝突不能強制送出，請調整開始日期、交期或拆單。",
        "warn",
      );
      return;
    }
    ...
    try {
      ...
      closePreviewPage();
      scheduledOrderIds.forEach((orderId) =>
        state.selectedOrderIds.delete(orderId),
      );
      if (payload.status === "queued" || payload.status === "running") {
        showMessage(
          "排程任務已送出",
          `任務 ${payload.id} 已進入背景排程，完成後會更新日曆。`,
        );
...
document
  .getElementById("close-preview-page")
  .addEventListener("click", closePreviewPage);
document
  .getElementById("preview-page-list")
  .addEventListener("click", async (event) => {
    ...
    try {
      ...
      if (action === "retry-suggested-start") {
        await retryPreview({
          startDate: suggestedStartDate(state.preview),
          manualForce: false,
          reason: "",
        });
        return;
      ...
      if (action === "update-conflict-due-date") {
        ...
        const input = document.querySelector(
          `[data-conflict-due-date="${cssEscape(orderId)}"]`,
        );
        if (!input?.value) {
      ...
      if (action === "unselect-conflict-order") {
        const orderId = event.target.dataset.orderId;
        state.selectedOrderIds.delete(orderId);
        const orderIds = state.preview.request.orderIds.filter(
          (id) => id !== orderId,
        );
      ...
      if (action === "preview-conflict-solution") {
        ...
        const resolutionOrderIds = checkedValues(
          "[data-conflict-resolution-order]",
        );
        if (orderIds.length === 0) {
          showMessage(
            "請選取訂單",
      ...
      if (action === "retry-manual-force") {
        ...
        const reason = document
          .getElementById("conflict-force-reason")
          .value.trim();
        if (!reason) {
          showMessage(
            "請填寫原因",
...
async function loadOrders() {
  ...
  state.selectedOrderIds = new Set(
    Array.from(state.selectedOrderIds).filter((id) =>
      selectableOrders().some((order) => order.id === id),
    ),
  );
  state.expandedSalesPendingOrderIds = new Set(
    Array.from(state.expandedSalesPendingOrderIds).filter((id) => {
...
async function loadCalendar() {
  ...
  const payload = await request(
    `/api/schedules/calendar?lineId=${encodeURIComponent(lineId)}&month=${encodeURIComponent(month)}`,
  );
  state.calendarAllocations = payload.allocations ?? [];
  state.pendingCalendarAllocations = (payload.pendingAllocations ?? []).map(
    (allocation) => ({ ...allocation, preview: true }),
  );
  renderCalendar();
}
...
async function loadScheduleHistory() {
  ...
  const lineId = activeLine();
  const payload = await request(
    `/api/schedules/history?lineId=${encodeURIComponent(lineId)}`,
  );
...
async function createPreview(requestData, kind) {
  ...
  state.preview = {
    ...
    request: {
      ...
      currentDate:
        result.currentDate ?? payloadData.currentDate ?? todayDateInputValue(),
      orderIds: payloadData.orderIds ?? [],
      resolutionOrderIds: payloadData.resolutionOrderIds ?? [],
      manualForce:
        payloadData.manualForce === "on" || payloadData.manualForce === true,
      allowLateCompletion:
        payloadData.allowLateCompletion === "on" ||
        payloadData.allowLateCompletion === true,
...
function renderAuthState() {
  ...
  document.getElementById("batch-bar").hidden =
  ...
  document.getElementById("active-line-select").disabled =
    state.user?.role === "scheduler";
  if (loggedIn) {
    document.getElementById("session-greeting").textContent =
      `您好 ${state.user.username}`;
  } else {
    closePreviewPage();
    closeProductionReport();
  }
...
function renderOrders() {
  const visibleOrders =
    state.user?.role === "sales"
      ...
      : visibleLineOrders();
  const filtered = sortOrdersForWorkstation(
...
function renderOrderCard(order) {
  ...
  const selectable =
    state.user?.role === "scheduler" && order.status === "待排程";
  const salesPendingEditable = canSalesEditPendingOrder(order);
  const salesPendingExpanded =
    salesPendingEditable && state.expandedSalesPendingOrderIds.has(order.id);
...
function attachPointerScheduleDrag(card, orderId) {
  ...
  card.addEventListener("pointermove", (event) => {
    if (
      !pointerScheduleDrag ||
      pointerScheduleDrag.pointerId !== event.pointerId
    ) {
      return;
    }
    const distance = Math.hypot(
      event.clientX - pointerScheduleDrag.startX,
      event.clientY - pointerScheduleDrag.startY,
    );
...
function onMouseScheduleDragMove(event) {
  ...
  const distance = Math.hypot(
    event.clientX - mouseScheduleDrag.startX,
    event.clientY - mouseScheduleDrag.startY,
  );
...
function calendarCellFromPoint(clientX, clientY) {
  return (
    document.elementFromPoint(clientX, clientY)?.closest?.(".calendar-day") ??
    null
  );
}
...
function renderCustomerFilter() {
  ...
  const activeLabel =
    options.find((option) => option.value === nextCurrent)?.label ?? "全部";
  container.innerHTML = "";
...
function renderCalendar() {
  ...
  grid.querySelectorAll("[data-calendar-order-id]").forEach((button) => {
    button.addEventListener("click", (event) => {
      event.stopPropagation();
      handleCalendarOrderClick(
        button.dataset.calendarOrderId,
...
function renderPreviewPage() {
  ...
  const canConfirmSchedule =
    state.preview?.kind === "schedule" &&
    state.user?.role === "scheduler" &&
    (conflicts.length === 0 ||
      (manualForce && conflictsCanBeManuallyForced(conflicts)));
  document.getElementById("preview-page-title").textContent =
    conflicts.length > 0 ? "衝突處理" : "訂單分配預覽";
  document.getElementById("preview-detail-title").textContent =
    conflicts.length > 0 ? "衝突清單" : "分配明細";
  document.getElementById("close-preview-page").hidden = false;
  pageList.innerHTML =
    [
      ...conflicts.map((conflict, index) =>
        renderConflictItem(conflict, index, manualForce),
      ),
...
function renderPreviewCalendar(allocations) {
  ...
  const conflictedOrderIds = new Set(
    conflicts.map((conflict) => conflict.orderId).filter(Boolean),
  );
  const previewAllocations =
    conflicts.length > 0 && !isSalesDraft ? [] : allocations;
  const markedPreviewAllocations = markConflictedPreviewAllocations(
    markMovedPreviewAllocations(previewAllocations),
    conflictedOrderIds,
  );
  const movedFromAllocations = buildMovedFromAllocations(
    markedPreviewAllocations,
  );
  const pendingAllocations = salesDraftPendingPreviewAllocations(
    markedPreviewAllocations,
    conflicts,
  );
  ...
  for (const day of monthGrid(year, monthIndex)) {
    ...
    const effectiveAllocations = dayAllocations.filter(
      (allocation) => !allocation.movedFromPreview,
    );
    const cell = document.createElement("div");
...
function salesDraftPendingPreviewAllocations(
  markedPreviewAllocations,
  conflicts = [],
) {
  const pendingAllocations = markedPreviewAllocations.map((allocation) => ({
    ...allocation,
    preview: true,
  }));
  ...
  const successfulAllocations = pendingAllocations.filter(
    (allocation) => !conflictedOrderIds.has(allocation.orderId),
  );
  if (successfulAllocations.length > 0) {
    return successfulAllocations;
  }
  return state.pendingCalendarAllocations.map((allocation) => ({
    ...allocation,
    preview: true,
  }));
}

function markConflictedPreviewAllocations(
  previewAllocations,
  conflictedOrderIds,
) {
...
function markMovedPreviewAllocations(previewAllocations) {
  ...
  for (const allocation of state.calendarAllocations) {
    ...
    existingTotals.set(
      existingKey,
      (existingTotals.get(existingKey) ?? 0) + Number(allocation.quantity ?? 0),
    );
  }
  ...
  for (const allocation of previewAllocations) {
    ...
    previewTotals.set(
      previewKey,
      (previewTotals.get(previewKey) ?? 0) + Number(allocation.quantity ?? 0),
    );
  }
  return previewAllocations.map((allocation) => {
...
function renderConflictItem(conflict, index = 0, withAcknowledgement = false) {
  ...
  const affected = conflict.affectedOrderIds?.length
    ? `影響：${conflict.affectedOrderIds.join(", ")}`
    : "無已知受影響訂單";
  ...
  const canAcknowledge =
    ...
    withAcknowledgement &&
    conflict.reason ===
      "existing allocations require manual review or reschedule";
  ...
  if (salesDraft) {
    const explanation =
      conflict.orderId === "PREVIEW-DRAFT"
...
function renderConflictSolutionPicker(conflicts, selectedOrderIds) {
  const conflictOrderIds = Array.from(
    new Set(conflicts.map((conflict) => conflict.orderId)),
  );
  const solutionOrderIds = Array.from(
    new Set([...(selectedOrderIds ?? []), ...conflictOrderIds]),
  ).sort(compareOrderIds);
  const selectedSet = new Set(selectedOrderIds ?? []);
  const hasSelection = selectedSet.size > 0;
  const affectedOrderIds = Array.from(
    new Set(conflicts.flatMap((conflict) => conflict.affectedOrderIds ?? [])),
  ).sort(compareOrderIds);
  const movableAffected = affectedOrderIds.filter(canMoveOrder);
  const blockedAffected = affectedOrderIds.filter(
    (orderId) => !canMoveOrder(orderId),
  );
  return `
...
function renderSolutionNotice(allocations) {
  ...
  const lateOrderIds = Array.from(
    new Set(lateAllocations.map((allocation) => allocation.orderId)),
  ).sort(compareOrderIds);
  return `
...
function canMoveOrder(orderId) {
  ...
  const allocations = state.calendarAllocations.filter(
    (allocation) => allocation.orderId === orderId,
  );
  return (
    allocations.length > 0 &&
    allocations.every(
      (allocation) => !allocation.locked && allocation.status === statuses[1],
    )
  );
}
...
function renderWaterline(allocations) {
  ...
  const remainingValue = metrics.overloaded
    ? metrics.total - metrics.capacity
    : metrics.remaining;
  return `
...
function renderCalendarItem(allocation) {
  ...
  const movedFromClass = allocation.movedFromPreview
    ? "moved-from-preview"
    : "";
  const quantityChangedClass = allocation.quantityChangedPreview
    ? "quantity-changed-preview"
    : "";
  const childOrderClass = isNewChildScheduledAllocation(allocation)
    ? "child-order-preview"
    : "";
  ...
  const attrs = actionable
    ...
    : "";
  const movedNote = allocation.movedFromPreview
    ? '<span class="calendar-item-note">已移出</span>'
    : "";
  const quantityNote = allocation.quantityChangedPreview
    ? '<span class="calendar-item-note">數量調整</span>'
    : "";
  const childNote = isNewChildScheduledAllocation(allocation)
    ? '<span class="calendar-item-note">子訂單</span>'
    : "";
  return `
...
function isNewChildScheduledAllocation(allocation) {
  ...
  return !state.calendarAllocations.some(
...
function calendarDisplayQuantity(allocation) {
  if (
    allocation.status === statuses[3] &&
    Number(allocation.completedQuantity ?? 0) > 0
  ) {
...
function renderOrderAction(order) {
  ...
  if (canSalesEditPendingOrder(order)) {
    ...
    return `
      ${
        expanded
          ? renderSalesCorrectionForm(order, {
              deleteLabel: "刪除訂單",
...
function renderSalesCorrectionForm(order, options = {}) {
  const contextLabel = options.contextLabel
    ? `<p class="modification-reason">${escapeHtml(options.contextLabel)}</p>`
    : "";
  ...
  try {
    ...
    if (action === "resubmit-order") {
      const card = document.querySelector(
        `[data-order-id="${cssEscape(orderId)}"]`,
      );
      const dueDate = card?.querySelector(
        '[data-resubmit-field="dueDate"]',
      )?.value;
      const quantity = Number(
        card?.querySelector('[data-resubmit-field="quantity"]')?.value,
      );
...
function openProductionReport(order, productionDate = "") {
  ...
  document.getElementById("production-context").textContent =
    `生產日期 ${dateOnly(allocation.date)}，本日排程 ${allocation.quantity.toLocaleString()} 片，交期 ${dateOnly(order.dueDate)}。完成片數不可大於本日排程量，未生產部分會保留同一訂單編號並回到待排程。`;
...
function productionAllocationForOrder(orderId, productionDate = "") {
  const allocations = state.calendarAllocations
    .filter(
      (allocation) =>
        allocation.orderId === orderId && allocation.status !== statuses[3],
    )
    .sort((a, b) => dateOnly(a.date).localeCompare(dateOnly(b.date)));
  if (productionDate) {
    return allocations.find(
      (allocation) => dateOnly(allocation.date) === productionDate,
    );
  }
...
async function submitProductionReport(
  ...
  if (producedQuantity > allocation.quantity) {
    showMessage(
      "片數超過本日排程",
      `完成片數不可大於 ${allocation.quantity.toLocaleString()} 片。`,
      "warn",
    );
    return;
  }
  try {
    ...
    closeProductionReport();
    const suffix = payload.remainder
      ? `，${payload.remainder.id} 剩餘 ${payload.remainder.quantity.toLocaleString()} 片已回到待排程`
      : "，已全數完成";
...
function updateSelectedCount() {
  document.getElementById("selected-count").textContent =
    `已選取 ${state.selectedOrderIds.size} 張訂單`;
}
...
function renderScheduleHistory() {
  ...
  list.innerHTML = state.scheduleHistory
    .map(
      (item) => `
...
async function submitRejectOrders() {
  ...
  try {
    ...
    state.rejectOrderIds.forEach((orderId) =>
      state.selectedOrderIds.delete(orderId),
    );
    closePreviewPage();
...
function scheduleFormData() {
  const data = Object.fromEntries(
    new FormData(document.getElementById("schedule-form")),
  );
...
function orderFormData() {
  syncLineInputs();
  const data = Object.fromEntries(
    new FormData(document.getElementById("order-form")),
  );
  data.lineId = activeLine();
...
function syncDueDateMinimums() {
  ...
  document
    .querySelectorAll(
      'input[name="dueDate"], [data-resubmit-field="dueDate"], [data-conflict-due-date]',
    )
    .forEach((input) => {
      input.min = min;
    });
...
async function waitForScheduleJob(jobId, attempts = 20) {
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    const job = await request(
      `/api/schedules/jobs/${encodeURIComponent(jobId)}`,
    );
...
function activeLineTimezone() {
  return (
    state.lines.find((line) => line.id === activeLine())?.timezone ||
    defaultTimezone
  );
}
