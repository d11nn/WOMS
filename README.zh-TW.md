...
- sales 可以把客戶訂單加入待排程；客戶訂單交期必須是所選產線 timezone 的明天或更晚日期，無效交期會顯示 `無法被接受的交期`。草稿可行性會對照既有已排程 allocation，並把同產線目前待排程 backlog 納入產能占用；preview dialog 會顯示這些待排程 allocation 與水位占用，也會標註因新草稿訂單而被推遲到交期後的待排程訂單，讓 sales 在確認新訂單時可將選取的衝突待排程訂單改送「需業務處理」。主月排程行事曆在確認前不會被改變，確認後待排程 overlay 會排除已移到需業務處理的訂單。scheduler 的正式排程流程仍只計算被選取的待排程訂單。
