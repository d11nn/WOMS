...
- Calendar orders shown on the same day are sorted by high priority first, then earlier due date, then earlier order-created Unix epoch milliseconds (`createdAtTimestamp`).
- Sales users can add customer orders to pending scheduling; customer order due dates must be tomorrow or later in the selected production line's timezone, and invalid due dates show `無法被接受的交期`. Draft feasibility is checked against existing scheduled allocations plus the current pending backlog for that line; the preview dialog shows those pending allocations and waterline usage, and it flags pending orders that the new draft would push past their due date. Conflict calendars show the successful part of the draft preview, including a new draft order that fits and excluding pending orders listed in the conflict report; if no draft preview allocation can be shown, they fall back to the current pending, scheduled, and all-order baseline views without adding the rejected draft. The main monthly calendar remains unchanged. Scheduler engineers' formal scheduling flow still considers only the selected pending orders.
