...
- 同一天的月曆訂單會依高優先級優先、交期較早優先、訂單建立時間 Unix epoch 毫秒數 (`createdAtTimestamp`) 較早優先排序。
- sales 可以把客戶訂單加入待排程；客戶訂單交期必須是所選產線 timezone 的明天或更晚日期，無效交期會顯示 `無法被接受的交期`。草稿可行性會對照既有已排程 allocation，並把同產線目前待排程 backlog 納入產能占用；preview dialog 會顯示這些待排程 allocation 與水位占用，也會標註因新草稿訂單而被推遲到交期後的待排程訂單。衝突日曆會顯示草稿試排中已成功排入的部分，包含可排入的新草稿訂單，並排除衝突清單中的待排程訂單；若沒有可顯示的草稿試排 allocation，才退回目前待排程、已排程與所有訂單的基準畫面，且不會加入被拒絕的新草稿。主月排程行事曆不會被改變。scheduler 的正式排程流程仍只計算被選取的待排程訂單。
